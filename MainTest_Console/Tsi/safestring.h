#ifndef __TENIO_SAFESTRING_H__
#define __TENIO_SAFESTRING_H__
...
#define _SNTPRINTF_S_IMPL()\
        ...
        strtype buffer, int nCount, int nMaxCountStore, conststrtype format, va_list argptr)
    {
        ...
        if (buffer != NULL
            && nCount > 0
            && (nMaxCountStore > 0 || nMaxCountStore == _TRUNCATE)
            && format != NULL && argptr != NULL)
        {
            int nSafeCount = ((nCount - 1) < nMaxCountStore || nMaxCountStore == _TRUNCATE)
                             ? (nCount - 1) : nMaxCountStore;
